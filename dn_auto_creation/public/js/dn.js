			...
			function () {
				erpnext.utils.map_current_doc({
					...
					get_query: function () {
						...
						// console.log(me.frm.doc.company)
						return {

							query: "dn_auto_creation.overrides.filter_dn.get_delivery_notes_to_be_billed",
							filters: filters
						};
					}
				});
...
frappe.ui.form.on('Sales Invoice', {
	on_submit: function (frm) {
		...
		if (frm.doc.is_return == 1) {
			...
			let uniquedn = [];
			arrays.forEach((c) => {
				if (!uniquedn.includes(c)) {
					uniquedn.push(c);
				}
			});


			for (let k = 0; k < uniquedn.length; k++) {
				var item = []
				$.each(frm.doc.items, function (j, e) {

					if (uniquedn[k] == e.delivery_note) {
				...
				let ticket = {
					...
					'taxes': [
						{
						"account_head":frm.doc.taxes[0].account_head ,
						"description":frm.doc.taxes[0].description,
						"charge_type":frm.doc.taxes[0].charge_type,
						"rate":frm.doc.taxes[0].rate
						}
				...
				frappe.db.insert(ticket).then((doc) => {
					...
					frappe.call({
						...
						"args": {
							...
							"name": uniquedn[k],
							"fieldname": {
								"branch": document_branch, "dn_return_no": document_name, 
								"dn_return_date": document_date,
								"returned": 1
							}
